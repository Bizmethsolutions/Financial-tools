...
<head>
  ...
  <script>
    ...
    jqOld(function() {
        jqOld("#start_date" ).datepicker({ dateFormat: 'yy-mm-dd' });
        jqOld("#fiscal_year" ).datepicker({ dateFormat: 'yy-mm-dd' });
        jqOld("#warrenty_end_date" ).datepicker({ minDate: 0,dateFormat: 'yy-mm-dd' });
    });
...
function checkbtn(text){
  if(text == 'Now'){
    $("#OpenImgUpload").prop('required',true);     
  }
  if(text == 'Later'){
    $("#OpenImgUpload").prop('required',false);     
  }
...
$('#fiscal_year').keypress(function(e) {
    e.preventDefault();
});
...
$('#step1').click(function(){ 
    ...
    let fiscal_date=$("#fiscal_year").val();
    if(fiscal_date == ''){
      $('#errorcn').hide();
      $('#errorc').hide();
      $('#errorc').hide();
  ...
  let fa=$("#fixed_account").val();
    ...
    $("#addassets > tbody"). empty();
      ...
      $("#fixed_account option:selected").each(function(){
        var selectedItem5 = $(this).text();
        if (selectedItem5) {
          faarray.push(selectedItem5); 
        }
      });
      ...
      $.ajax({
         ...
         url:"{{ url('step2') }}",
         data:{
            "fixed_assets":faarray, 
            "accumulated":outArray, 
            "track":trackarray, 
            "gl":glarray,
            "decimal_round":decimal,
            "deminus_amount":deminus,
            "_token": "{{ csrf_token() }}"
          },
         success:function(data){
